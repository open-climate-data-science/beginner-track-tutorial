...
<center> 
<font size="24">Download the [raw_cardinal_data.csv](raw_cardinal_data.csv). 
 </font>
\center
...
- This is a scatterplot showing the distribution of average leaf wetness across all years. 
- The highest point is in January 2021.
...
```{r}
library(ggplot2)
ggplot(cardinal,aes(x=month,y=Tprep))+geom_boxplot()
```
...
```{r}
ggplot(cardinal,aes(x=date,y=AvgT))+geom_line()+labs(title="Total Average Temperature by Date",y="Average Tempurature (F) ", x= "Date")
```
...
- This histogram shows the average temperature over the days.
...
- This scatterplot shows the average humidity vs average leaf wetness by month. As humidity increases, average leaf wetness increases.
...
Here we are using the ECOnet Data, making a paragraph. 
